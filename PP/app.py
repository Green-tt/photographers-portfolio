...
# Файл для хранения информации "Обо мне"
ABOUT_FILE = Path("about_data.json")
...
if not ABOUT_FILE.exists():
    with open(ABOUT_FILE, 'w') as f:
        json.dump({
            "title": "About",
            "content": [
                "Hello, I'm Tommaso Bacchelli, a creative professional specializing in visual design and digital experiences. With over a decade of experience, I've worked with clients ranging from startups to global brands, helping them bring their ideas to life through thoughtful design.",
                "My approach combines aesthetic sensibility with functional clarity, creating work that is both beautiful and effective. I believe in the power of simplicity and the importance of storytelling in design.",
                "When I'm not designing, you can find me exploring new technologies, traveling, or experimenting with photography. I'm passionate about the intersection of art and technology and how it shapes our daily experiences."
            ],
            "contact_title": "Get in Touch",
            "contact_text": "I'm always open to discussing new projects, creative ideas, or opportunities to be part of your vision. Feel free to reach out through the contact form or directly via email.",
            "email": "hello@tommasobacchelli.com",
            ...
            "copyright": "© 2023 Tommaso Bacchelli. All rights reserved.",
...
def load_about_data():
    try:
        ...
        return {
            "title": "About",
            "content": ["Default content 1", "Default content 2", "Default content 3"],
            "contact_title": "Contact",
            "contact_text": "Default contact text",
...
# Маршрут для редактирования информации (только для админа)
@app.get("/admin/edit_about", response_class=HTMLResponse)
async def edit_about_page(request: Request):
    if not is_admin(request):
        return RedirectResponse(url="/login?next=/admin/edit_about", status_code=303)

    about_data = load_about_data()

    return templates.TemplateResponse("edit_about.html", {
        "request": request,
        "user": get_current_user(request),
        "about_data": about_data
    })
...
async def update_about(
    ...
    if not is_admin(request):
        raise HTTPException(status_code=403, detail="Только администратор может редактировать информацию")
    ...
    save_about_data(about_data)

    return RedirectResponse(url="/about_me", status_code=303)
...
async def index(request: Request):
    return templates.TemplateResponse("about_me.html", {
        ...
        "user": get_current_user(request),
        "is_admin": is_admin(request),
        "about_data": load_about_data()  # <-- ДОБАВЬТЕ ЭТУ СТРОКУ
    })
...
async def about_me(request: Request):
    return templates.TemplateResponse("about_me.html", {
        ...
        "user": get_current_user(request),
        "is_admin": is_admin(request),
        "about_data": load_about_data()  # <-- ДОЛЖНО БЫТЬ!
    })
